package br.com.phc.pitaco.gateway.handler;
...
public class GatewayErrorAttributes extends DefaultErrorAttributes {
	...
	public Map<String, Object> getErrorAttributes(ServerRequest request, ErrorAttributeOptions options) {
		...
		map.put("message", this.populateMapWithErrorMessage(error, map, status));
		map.put("uniqueId", UUID.randomUUID().toString());
		map.put("informationCode", status.name());
	...
	private HttpStatus getHttpStatusFromStatusCode(int statusCode) {
		return Arrays.stream(HttpStatus.values()).filter(v -> v.value() == statusCode)//
				.findFirst()//
	...
	private String trimStatusAndQuotesFromErrorMessage(String errorMessage, HttpStatus status) {
		return errorMessage.replace(status.toString(), "").replace("\"", "").trim();
	}
