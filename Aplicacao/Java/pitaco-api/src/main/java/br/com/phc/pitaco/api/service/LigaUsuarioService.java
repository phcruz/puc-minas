...
public class LigaUsuarioService {
	...
	public void inserirLigaUsuario(Long idLiga, Long idUsuario) {
		...
		LigaUsuario l = this.criaLigaUsuario(idLiga, idUsuario);
		this.insereLigaUsuario(l);
	}
	...
	public void alterarLigaUsuario(LigaUsuarioDTO ligaUsuarioDTO) {
		log.info("Acessando alterarLigaUsuario(LigaUsuario novaLigaUsuario)");
		Calendar data = Calendar.getInstance();

		LigaUsuario liga = this.buscarLigaUsuario(ligaUsuarioDTO);
		if (liga != null && liga.getIdLigaUsuario() > 0) {
			...
			liga.setLiga(new Liga(ligaUsuarioDTO.getIdLiga()));
			liga.setUsuario(new Usuario(ligaUsuarioDTO.getIdUsuario()));
			liga.setAtivo(true);
			liga.setDataCadastro(data.getTime());
			liga.setDataAlteracao(data.getTime());
			this.insereLigaUsuario(liga);
		}
	}
	
	public LigaUsuario buscarLigaUsuario(LigaUsuarioDTO ligaUsuarioDTO) {
		log.info("Acessando buscarLigaUsuario(LigaUsuario ligaUsuario)");
		return ligaUsuarioRepository.buscarLigaUsuario(ligaUsuarioDTO);
	}
}
